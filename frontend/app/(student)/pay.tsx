...
export default function PayScreen() {
  ...
  const merchants = Object.entries(MERCHANT_CATEGORIES).map(([id, name]) => ({
    id,
    name,
    icon: id.includes("CAFE")
      ? "restaurant"
      : id.includes("LIBRARY")
        ? "book"
        : id.includes("STATIONARY")
          ? "storefront"
          : "cafe",
  }));
  ...
  const handleGenerateQR = async () => {
    ...
    if (payAmount > balance) {
      Alert.alert(
        ...
        `You only have ₹${balance.toFixed(2)} in your wallet`,
      );
      return;
    }
    ...
    setGenerating(true);
    try {
      const data = await apiRequest<{ qrPayload: any }>(
        "/api/transactions/generate-qr",
        {
          method: "POST",
          body: JSON.stringify({
            amount: payAmount,
      ...
      // Backend returns signed payload (includes HMAC signature)
  ...
  if (qrGenerated && qrData) {
    ...
    const merchantIcon =
      merchants.find((m) => m.id === paymentInfo.merchantId)?.icon ||
      "storefront";
        ...
        <View style={styles.qrCard}>
          ...
          <View style={styles.merchantInfo}>
            <View style={styles.merchantIconSmall}>
              <Ionicons
                name={merchantIcon as any}
                size={24}
                color={colors.merchant || colors.primary}
              />
            </View>
            <View>
        ...
        <View style={styles.detailsCard}>
          ...
          <View style={styles.detailRow}>
            ...
            <Text
              style={[
                styles.detailValue,
                { color: colors.student || colors.primary },
              ]}
            >
            ...
            >
              ...
              >
                <Ionicons
                  ...
                  size={24}
                  color={
                    selectedMerchant === merchant.id ? "#fff" : colors.primary
                  }
                />
              </View>
              <View style={{ flex: 1 }}>
      ...
      <View style={styles.section}>
        <TouchableOpacity
          style={[
            ...
            (!amount || !selectedMerchant || generating) &&
              styles.generateButtonDisabled,
          ]}
          onPress={handleGenerateQR}
          activeOpacity={0.8}
          disabled={!amount || !selectedMerchant || generating}
        >
          {generating ? (
...
const getStyles = (colors: any) =>
  StyleSheet.create({
    ...
    merchantCardActive: {
      borderColor: colors.student || colors.primary,
      backgroundColor: (colors.student || colors.primary) + "10",
    },
    merchantIcon: {
      width: 48,
      height: 48,
